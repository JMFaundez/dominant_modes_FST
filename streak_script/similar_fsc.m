...
c=sqrt((mfac+1)/2);
v=-(f(:,1)+(mfac-1)/(mfac+1)*f(:,2).*yt)*c;
vy=-(f(:,2)+(mfac-1)/(mfac+1)*f(:,2)+(mfac-1)/(mfac+1)*f(:,3).*yt)*c;
Ufsc.u=interp1(yt,f(:,2),y,'pchip');
Ufsc.w=interp1(yt,f(:,4)/f(end,4),y,'pchip');
Ufsc.v=interp1(yt,v,y,'pchip');
Ufsc.uy=interp1(yt,f(:,3),y,'pchip');
Ufsc.wy=interp1(yt,f(:,5)/f(end,4),y,'pchip');
Ufsc.vy=interp1(yt,vy,y,'pchip');
